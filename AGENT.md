...
- Tracks the joint drift of gas-line superheat (`Tg − Te`, THMg gas-pipe thermistor, a condensing-side lift ~40-60 K in heating, not a suction superheat) and outdoor expansion-valve opening (`EVO`) against a per-installation baseline learned over ~2 weeks; gas-line superheat is the season-robust primary signal, `EVO` is compared only at equivalent outdoor temperature.
- Surfaces `sensor.*_refrigerant_charge_status` (ENUM `learning`/`ok`/`watch`/`alert`), a `Reset Refrigerant Baseline` button, and a fixable repair issue on sustained alert. In season the issue clears on recovery; off-season a frozen alert does not self-clear (a slow loss does not repair itself), so it is annotated as stale (`last_valid_day`/`days_since_valid_data` attributes) and offers a "circuit was serviced" fix flow that resets the baseline. Baseline persists via a HA `Store`. **Opt-in and off by default** (`CONF_REFRIGERANT_DETECTION`), surfaced as one toggle of the "Preventive maintenance" section in the "Advanced features (beta)" panel, in the setup flow and the options flow, plus an onboarding repair issue (`enable_refrigerant_detection_{entry_id}`, `EnableRefrigerantDetectionRepairFlow`) for existing installs on capable profiles. The detector, its sensor, the reset button and the repair issue all require both the capability (`supports_extended_compressor_sensors`, so never the Yutampo R32) and the user's consent (`coordinator.refrigerant_detection_active`).
- See [docs/reference/refrigerant-monitoring.md](docs/reference/refrigerant-monitoring.md).

### Compressor Short-Cycling Detection
- Local, opt-in early warning for a compressor that starts and stops too often in **space heating** (`domain/services/cycling.py`, `CyclingMonitor`). Advisory only.
- **Second detection pattern of the domain**: an *absolute threshold per regime*, with **no learned baseline**, because short cycling is a stable state rather than a drift (a baseline would learn the fault). The refrigerant detector remains the reference for the *learned baseline, relative drift* pattern. Ask of any new detector: is this a drift, or a state?
- A day is valid at 4+ measured cycle periods and faulted when **both** `peak starts >= 6 per 60 min` **and** `median cycle period <= 15 min`; 3 consecutive faulted days raise a non-fixable repair issue that clears on its own, and the verdict **expires after 14 calendar days without a valid heating day** (`STALE_AFTER_DAYS`) so an alert does not freeze all summer, unlike the refrigerant one which must not self-clear. Thresholds calibrated by replaying the telemetry archive (the literature's 3 starts/h flags a third of the fleet).
- Measurement hygiene: 30 s debounce, data-hole guard (4x median poll interval, 90 s floor), defrost guard, heating-only regime, first observed state never counted as a start. Intra-day buffers are deliberately not persisted; daily aggregates and the streak live in a HA `Store`.
- Surfaces `sensor.*_compressor_cycling`. **Opt-in, off by default** (`CONF_CYCLING_DETECTION`), no capability gate (every profile has a compressor).
- See [docs/reference/cycling-monitoring.md](docs/reference/cycling-monitoring.md).

### Preventive Maintenance Consent
- The beta detectors are grouped in a **"Preventive maintenance" section** of the "Advanced features (beta)" panel, with **one toggle per detector** (`_preventive_maintenance_schema` in `config_flow.py`). The panel is reached by every profile; a toggle is hidden where its capability is missing.
- A config-flow `section` nests its values under the section key, so `_flatten_preventive_maintenance` **flattens them on write**: options stay flat (`refrigerant_detection`, `cycling_detection`) and no options migration is needed. Keep it that way, presentation must not dictate storage.
- See [docs/reference/preventive-maintenance.md](docs/reference/preventive-maintenance.md).

### Anonymous Telemetry
- Binary consent (Off / On) stored in `entry.options["telemetry_level"]` (`CONF_TELEMETRY_LEVEL`, default `"off"`).
- **Package** (`telemetry/`): models (three payloads: `InstallationInfo`, `MetricsBatch`, `RegisterSnapshot`), collector (circular buffer), anonymizer (SHA-256 hash, temperature/geolocation rounding; `hash_instance_id` for the per-household `instance_hash` and `hash_device_id` for the per-unit `device_hash`, derived from `f"{instance_id}:{entry.unique_id}"`), HTTP client (gzip + retry), noop client.
- **Buffering**: the collector's buffer holds 30 minutes of points; `flush()` returns at most `MAX_FLUSH_POINTS` (80) of them, oldest first, and leaves the rest buffered so a backlog cannot grow past the endpoint's 256 KB payload limit (at the cap a batch is ~88 KB on the narrowest profile and ~187 KB on the widest, so the margin is 27% in the worst case). A failed send re-queues its points (`requeue`, dropping anything older than `MAX_POINT_AGE`); the clients return a `SendResult` rather than a bool so the coordinator drops, instead of retrying, a batch refused with HTTP 413.
...
- **Backend**: Cloudflare Worker (ingestion/validation/rate-limit per payload type) → R2 (permanent JSON archive, partitioned Hive-style). Both the rate limit and the R2 archive keys are keyed per unit (`device_hash`), not per household, so a multi-gateway installation no longer has its units starve each other's rate-limit budget. Installation payloads are mirrored to Workers Analytics Engine (dataset `hitachi_installations`) for a Grafana fleet dashboard, which keeps `instance_hash` as its index (`blob1`, unchanged, so existing panels keep working) and appends `device_hash` as `blob8`; the integration re-sends installation daily to keep WAE's 90-day window populated.
...
- [Reference](docs/reference/): entity reference, entity patterns, domain services, preventive maintenance (refrigerant, cycling), quality scale, telemetry, model nomenclature.
