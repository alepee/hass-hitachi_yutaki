...
- A day is valid at 4+ measured cycle periods and faulted when **both** `peak starts >= 6 per 60 min` **and** `median cycle period <= 15 min`; 3 consecutive faulted days raise a non-fixable repair issue that clears on its own. Thresholds calibrated by replaying the telemetry archive (the literature's 3 starts/h flags a third of the fleet).
