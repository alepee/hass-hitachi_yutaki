...
- **Buffering**: the collector's buffer holds 30 minutes of points; `flush()` returns at most `MAX_FLUSH_POINTS` (80) of them, oldest first, and leaves the rest buffered so a backlog cannot grow past the endpoint's 256 KB payload limit. A failed send re-queues its points (`requeue`, dropping anything older than `MAX_POINT_AGE`); the clients return a `SendResult` rather than a bool so the coordinator drops, instead of retrying, a batch refused with HTTP 413.
