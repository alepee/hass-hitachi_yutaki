...
`entry.unique_id` is guaranteed non-`None` at the point telemetry is wired: the
back-fill at `__init__.py:310-358` runs before the telemetry setup at
`__init__.py:400-432`. No degraded path is needed.
...
This is a consistency fix, not a live bug: Home Assistant enforces `unique_id`
uniqueness within a domain, so two live entries cannot already share one, and no
`device_hash` collision is possible in practice. Entries that back-filled under a
previous version keep their existing `unique_id` (the back-fill only runs when it
is `None`) and are unaffected.
